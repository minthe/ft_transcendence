...
function joinTourn(tournId, matches) {
	...
	matches.forEach(match => {
		if (match[0][0].tourn_host === tournId) {
			playerOne.textContent = match[1][0].player_one;
			playerTwo.textContent = match[1][0].player_two;
			playerThree.textContent = match[2][0].player_one;
			playerFour.textContent = match[2][0].player_two;
			
			if (match[1][0].winner_id) {
				winnerGameOne.textContent = match[1][0].winner_id;
			...
			if (match[2][0].winner_id) {
				winnerGameTwo.textContent = match[2][0].winner_id;
			...
			else if (match[1][0].winner_id && match[2][0].winner_id 
				&& (match[1][0].winner_id === userId || match[2][0].winner_id === userId)) {
